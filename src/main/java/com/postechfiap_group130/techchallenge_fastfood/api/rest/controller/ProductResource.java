...
public class ProductResource {
    ...
    public ResponseEntity<ProductDto> getProductById(@PathVariable UUID id) {
        ...
        ProductDto productResponseDto = productController.getProductById(id);
    ...
    public ResponseEntity<ProductDto> getProductByName(@PathVariable String name) {
        ProductController productController = new ProductController(dataRepository);
        ProductDto productResponseDto = productController.getProductByName(name);
        if (productResponseDto == null) return ResponseEntity.status(HttpStatus.NOT_FOUND).body(null);
        return ResponseEntity.status(HttpStatus.OK).body(productResponseDto);
    }
    @GetMapping("/category/{category}")
    public ResponseEntity<List<ProductDto>> getProductByCategory(@PathVariable Product.Category category) {
        ProductController productController = new ProductController(dataRepository);
        List<ProductDto> productResponseDto = productController.getProductsByCategory(category);
        return ResponseEntity.status(HttpStatus.CREATED).body(productResponseDto);
    }

    @PostMapping("/create")
    public ResponseEntity<ProductDto> create(@RequestBody ProductRequestDto productRequestDto) throws DomainException {
        ProductDto produtoDto = new ProductDto(UUID.randomUUID(), productRequestDto.getName(),
                ...
                true);
        ProductController productController = new ProductController(dataRepository);
        ProductDto productResponseDto = productController.createProduct(produtoDto);
        return ResponseEntity.status(HttpStatus.CREATED).body(productResponseDto);
    }

    @PutMapping("/update")
    public ResponseEntity<ProductDto> updateProduct(@RequestBody ProductDto updateProdutoDto) throws DomainException {
        ProductController productController = new ProductController(dataRepository);
